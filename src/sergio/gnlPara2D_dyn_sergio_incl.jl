...
# File properties
filename = datadir("sims_sergio_incl","run","gnl")
...
# Straight
fx_X(r) = r[1] * cosd(θDeg)
fy_X(r) = r[1] * sind(θDeg)
...
function gFairLead(x,t)    
  ...
  fω = 2*pi*fFreq
  return VectorValue( 
    1.0 * cosd(θDeg) + tRamp*fAmp*sin(fω*t) * sind(θDeg), 
    1.0 * sind(θDeg) - tRamp*fAmp*sin(fω*t) * cosd(θDeg) )
end   
...
solnht = solve(ode_solver, op_AD, t0, simT, (u0,u0t)) 
