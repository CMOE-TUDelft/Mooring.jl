...
# end
map(r) = r#VectorValue(r[1],r[1]^2)
material1 = M.LinearElastic(E=1.0e3)
material2 = M.LinearElastic(E=1.0e0)

# Create points
pointAmotion_function = (t,x) -> VectorValue(p1x,p1y)
...
pointC = Pts.MooringPoint(model, "PointC", pointCmotion)

# Create a segments
segment1 = Seg.MooringSegment(model, "Line1", pointA, pointB, map, material1, 10.0)
segment2 = Seg.MooringSegment(model, "Line2", pointB, pointC, map, material2, 1.0)
